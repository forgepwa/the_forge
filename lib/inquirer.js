...
module.exports = {
    ...
    const QUESTIONS = [
      {
        ...
        type: "input",
        message:
          "What is the name of your Firebase project? (this must match the name on your firebase console)",
        validate(input) {
      ...
      },
      {
        ...
        validate(input) {
          if (fs.readdirSync(process.cwd()).indexOf(input) !== -1)
  ...
  askTemplate: async () => {
    // Grab templates and filter out hidden files
    const CHOICES = fs.readdirSync(`${__dirname}/templates`).filter((file) => {
    ...
    const QUESTIONS = [
      ...
      },
      {
        ...
        validate(input) {
          if (fs.readdirSync(process.cwd()).indexOf(input) !== -1)
            return "Project name must be unique.";
          if (/^([A-Za-z\-_\d])+$/.test(input) && input.length >= 4)
            return true;
