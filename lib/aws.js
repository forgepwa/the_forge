...
module.exports = {
  ...
  async ebCreate(projectName) {
    const status = new Spinner("eb create ...");
    status.start();
    await cmd.get(`eb create ${projectName}`, err => {
      if (!err) {
        cmd.get("eb open", err => {
          if (!err) {
            status.stop();
          } else console.log("EB OPEN ERROR", err);
        });
      } else console.log("EB CREATE ERROR", err);
    });
  },
  ...
  createEnvironment(projectDirName, projectName) {
    const params = {
      ApplicationName: projectName,
      EnvironmentName: "pwa-app1",
