...
module.exports = {
  ...
  async createCLI(projectName) {
    ...
    // status.start();
    console.log('Sorry our spinner is broken. Creating Elastic Beanstalk application and evironment. This may take a few minutes to complete...');
    try {
      cmd.get(`eb create ${projectName}`, (err, data) => {
        if (!err) {
          console.log('Created your project! Opening in your browser...');
          cmd.get('eb open');
        } else {
  ...
  deploy() {
    const status = new Spinner('Redeploying your project to AWS. This may take a few minutes to complete...');
    status.start();
    try {
