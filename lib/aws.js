...
module.exports = {
  ...
  async createCLI(projectName) {
    ...
    status.start();
    const asyncCLI = Promise.promisify(cmd.get, {
      multiArgs: true,
      context: cmd
    });

    asyncCLI(`eb create ${projectName}`)
      .then(data => {
        console.log("cmd data", data);
      })
      .catch(err => {
        status.stop();
        console.log("Could not deploy your project to AWS. You may need to logout with forge -o and reauthenticate your AWS credentials.")
        this.AWSLogout();
      });
  },
