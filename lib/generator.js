...
module.exports = {
  ...
  generateEBConfig(projectName){
    try{
      const ebconfigPath = `${__dirname}/init/SSL`;
      this.createDirectoryContents(ebconfigPath, projectName).catch(err => {console.log("caught error", err)});
      commands.changeDir(projectName).catch(err => {console.log("caught error", err)});
    }catch (err) {
      throw new Error(`Could not create https configuration files for this project.\nError: ${err}`);
    }
